...
@php
    ...
    $fields = json_decode('[{"label":"Sequence Name","name":"name","placeholder":"Sales Invoice Chennai","type":"text"},{"label":"Prefix","name":"prefix","placeholder":"CHN-SI-","type":"text"},{"label":"Next Number","name":"next_number","placeholder":"1001","type":"number"},{"label":"Padding","name":"padding","placeholder":"4","type":"number"},{"label":"Reset Period","name":"reset_period","options":["Never","Monthly","Financial Year"],"type":"select"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('invoice-sequences.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('invoice-sequences.create')<a class="btn btn-primary" href="{{ route('admin.invoice-sequences.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.invoice-sequences.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.invoice-sequences.pdf')) + '?' + params;
    });
});
</script>
@endpush