...
@php
    ...
    $fields = json_decode('[{"label":"Variant Name","name":"name","placeholder":"25 KG Bag","type":"text"},{"label":"Variant Type","name":"variant_type","options":["Packing","Color","Size","Quality"],"type":"select"},{"label":"Code","name":"code","placeholder":"VAR-25KG","type":"text"},{"label":"Sort Order","name":"sort_order","placeholder":"1","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Variant details","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('variants.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('variants.create')<a class="btn btn-primary" href="{{ route('admin.variants.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.variants.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.variants.pdf')) + '?' + params;
    });
});
</script>
@endpush