...
@php
    ...
    $fields = json_decode('[{"label":"Configuration Name","name":"name","placeholder":"GST Regular","type":"text"},{"label":"Tax Type","name":"tax_type","options":["GST","IGST","CESS","None"],"type":"select"},{"label":"Registration Number","name":"registration_number","placeholder":"GSTIN","type":"text"},{"label":"Effective From","name":"effective_from","type":"date"},{"label":"Default Tax Rate","name":"default_rate","placeholder":"18","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('tax-configurations.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('tax-configurations.create')<a class="btn btn-primary" href="{{ route('admin.tax-configurations.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.tax-configurations.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.tax-configurations.pdf')) + '?' + params;
    });
});
</script>
@endpush