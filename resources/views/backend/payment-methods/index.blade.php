...
@php
    ...
    $fields = json_decode('[{"label":"Method Name","name":"name","placeholder":"UPI","type":"text"},{"label":"Code","name":"code","placeholder":"UPI","type":"text"},{"label":"Settlement Account","name":"settlement_account","placeholder":"Default bank/cash account","type":"text"},{"label":"Charges %","name":"charges_percent","placeholder":"0","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Notes","name":"notes","placeholder":"Usage notes","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('payment-methods.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('payment-methods.create')<a class="btn btn-primary" href="{{ route('admin.payment-methods.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.payment-methods.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.payment-methods.pdf')) + '?' + params;
    });
});
</script>
@endpush