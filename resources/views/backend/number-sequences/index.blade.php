...
@php
    ...
    $fields = json_decode('[{"label":"Sequence Name","name":"name","placeholder":"Receipt Number","type":"text"},{"label":"Module","name":"module","placeholder":"Payments","type":"text"},{"label":"Prefix","name":"prefix","placeholder":"REC-","type":"text"},{"label":"Current Number","name":"current_number","placeholder":"1","type":"number"},{"label":"Suffix","name":"suffix","placeholder":"","type":"text"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('number-sequences.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('number-sequences.create')<a class="btn btn-primary" href="{{ route('admin.number-sequences.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.number-sequences.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.number-sequences.pdf')) + '?' + params;
    });
});
</script>
@endpush