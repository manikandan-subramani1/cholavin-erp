...
@php
    ...
    $fields = json_decode('[{"label":"Route Name","name":"name","placeholder":"Chennai North","type":"text"},{"label":"Route Code","name":"code","placeholder":"RT-CHN-N","type":"text"},{"label":"Start Point","name":"start_point","placeholder":"Godown","type":"text"},{"label":"End Point","name":"end_point","placeholder":"Area","type":"text"},{"label":"Estimated KM","name":"estimated_km","placeholder":"25","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Notes","name":"notes","placeholder":"Route notes","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('delivery-routes.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('delivery-routes.create')<a class="btn btn-primary" href="{{ route('admin.delivery-routes.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.delivery-routes.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.delivery-routes.pdf')) + '?' + params;
    });
});
</script>
@endpush