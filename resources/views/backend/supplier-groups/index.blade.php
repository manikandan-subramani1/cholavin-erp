...
@php
    ...
    $fields = json_decode('[{"label":"Group Name","name":"name","placeholder":"Rice Suppliers","type":"text"},{"label":"Code","name":"code","placeholder":"SGR-RICE","type":"text"},{"label":"Default Payment Days","name":"payment_days","placeholder":"15","type":"number"},{"label":"Purchase Category","name":"purchase_category","placeholder":"Raw material","type":"text"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Supplier group notes","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('supplier-groups.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('supplier-groups.create')<a class="btn btn-primary" href="{{ route('admin.supplier-groups.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.supplier-groups.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.supplier-groups.pdf')) + '?' + params;
    });
});
</script>
@endpush