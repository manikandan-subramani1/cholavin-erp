...
@php
    ...
    $fields = json_decode('[{"label":"Group Name","name":"name","placeholder":"Retail Customers","type":"text"},{"label":"Code","name":"code","placeholder":"CGR-RET","type":"text"},{"label":"Default Credit Days","name":"credit_days","placeholder":"7","type":"number"},{"label":"Default Credit Limit","name":"credit_limit","placeholder":"0.00","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Customer group notes","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('customer-groups.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('customer-groups.create')<a class="btn btn-primary" href="{{ route('admin.customer-groups.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.customer-groups.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.customer-groups.pdf')) + '?' + params;
    });
});
</script>
@endpush