...
<div class="card erp-panel">
    <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
        ...
        <a href="{{ route('admin.tax-rates.index') }}" class="btn btn-secondary"><i class="ri-arrow-left-line me-1"></i>Back</a>
    </div>
    <form action="{{ route('admin.tax-rates.update', $recordId) }}" method="POST" id="form-validate" data-index-url="{{ route('admin.'.$module['slug'].'.index') }}">
        @csrf
        ...
        <div class="card-body">
            <div class="row gy-4">
                <div class="col-xxl-3 col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control reference-master-input name-field @error('name') is-invalid @enderror" id="name" name="name" type="text" value="{{ old('name', $values['name'] ?? '') }}" placeholder="GST 5%" maxlength="190" required>
                        @error('name')
                ...
                <div class="col-xxl-3 col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control reference-master-input rate-field @error('rate') is-invalid @enderror" id="rate" name="rate" type="number" value="{{ old('rate', $values['rate'] ?? '') }}" placeholder="5" min="0" step="any" required>
                        @error('rate')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-xxl-3 col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-select reference-master-input tax-type-field @error('tax_type') is-invalid @enderror" id="tax_type" name="tax_type" required>
                            ...
                            <option value="CGST + SGST" @selected((string) old('tax_type', $values['tax_type'] ?? '') === 'CGST + SGST')>CGST + SGST</option>
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control reference-master-input notes-field @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3" maxlength="2000" placeholder="Tax applicability">{{ old('notes', $values['notes'] ?? '') }}</textarea>
                        @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mt-4 d-flex justify-content-end gap-2">
                <a href="{{ route('admin.tax-rates.index') }}" class="btn btn-secondary">Cancel</a>
