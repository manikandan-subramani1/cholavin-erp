...
@php
    ...
    $fields = json_decode('[{"label":"Driver Name","name":"name","placeholder":"Driver name","type":"text"},{"label":"Mobile Number","name":"mobile","placeholder":"Mobile","type":"text"},{"label":"License Number","name":"license_number","placeholder":"DL number","type":"text"},{"label":"License Expiry","name":"license_expiry","type":"date"},{"label":"Assigned Vehicle","name":"assigned_vehicle","placeholder":"Vehicle number","type":"text"},{"label":"Status","name":"status","options":["Active","Inactive","On Leave"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('drivers.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('drivers.create')<a class="btn btn-primary" href="{{ route('admin.drivers.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.drivers.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.drivers.pdf')) + '?' + params;
    });
});
</script>
@endpush