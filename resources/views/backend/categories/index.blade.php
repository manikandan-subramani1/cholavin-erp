...
@php
    ...
    $fields = json_decode('[{"label":"Category Name","name":"name","placeholder":"Rice","type":"text"},{"label":"Code","name":"code","placeholder":"CAT-RICE","type":"text"},{"label":"Display Order","name":"display_order","placeholder":"1","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Category description","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('categories.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('categories.create')<a class="btn btn-primary" href="{{ route('admin.categories.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.categories.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.categories.pdf')) + '?' + params;
    });
});
</script>
@endpush