...
@php
    ...
    $fields = json_decode('[{"label":"Unit Name","name":"name","placeholder":"Kilogram","type":"text"},{"label":"Short Code","name":"code","placeholder":"KG","type":"text"},{"label":"Decimal Places","name":"decimal_places","placeholder":"3","type":"number"},{"label":"Base Unit","name":"base_unit","placeholder":"Gram","type":"text"},{"label":"Conversion Value","name":"conversion_value","placeholder":"1000","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('units.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('units.create')<a class="btn btn-primary" href="{{ route('admin.units.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.units.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.units.pdf')) + '?' + params;
    });
});
</script>
@endpush