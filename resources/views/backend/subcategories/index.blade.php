...
@php
    ...
    $fields = json_decode('[{"label":"Parent Category","name":"category","options":["Rice","Pulses","Oil","Spices"],"type":"select"},{"label":"Subcategory Name","name":"name","placeholder":"Ponnu Rice","type":"text"},{"label":"Code","name":"code","placeholder":"SUB-PONNI","type":"text"},{"label":"Display Order","name":"display_order","placeholder":"1","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Subcategory description","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('subcategories.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('subcategories.create')<a class="btn btn-primary" href="{{ route('admin.subcategories.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.subcategories.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.subcategories.pdf')) + '?' + params;
    });
});
</script>
@endpush