...
@php
    ...
    $fields = json_decode('[{"label":"Grade Name","name":"name","placeholder":"Premium","type":"text"},{"label":"Code","name":"code","placeholder":"GR-PREM","type":"text"},{"label":"Quality Score","name":"quality_score","placeholder":"100","type":"number"},{"label":"Sort Order","name":"sort_order","placeholder":"1","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Description","name":"description","placeholder":"Grade specification","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('grades.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('grades.create')<a class="btn btn-primary" href="{{ route('admin.grades.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.grades.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.grades.pdf')) + '?' + params;
    });
});
</script>
@endpush