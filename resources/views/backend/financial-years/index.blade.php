...
@php
    ...
    $fields = json_decode('[{"label":"Year Name","name":"name","placeholder":"FY 2026-27","type":"text"},{"label":"Code","name":"code","placeholder":"FY-2026","type":"text"},{"label":"Start Date","name":"start_date","type":"date"},{"label":"End Date","name":"end_date","type":"date"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"},{"label":"Notes","name":"notes","placeholder":"Remarks","type":"textarea"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('financial-years.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('financial-years.create')<a class="btn btn-primary" href="{{ route('admin.financial-years.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.financial-years.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.financial-years.pdf')) + '?' + params;
    });
});
</script>
@endpush