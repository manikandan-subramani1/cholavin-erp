...
@section('content')
<div id="report-module" data-index-url="{{ route('admin.reports.index', $report) }}" data-pdf-url="{{ route('admin.reports.pdf', $report) }}">
    <div class="card erp-panel mb-3"><div class="card-body d-flex justify-content-between align-items-center gap-3"><div><span class="erp-eyebrow">Reports</span><h4 class="mb-0">{{ $title }}</h4></div>@can('reports.export')<button id="report-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>Download PDF</button>@endcan</div></div>
