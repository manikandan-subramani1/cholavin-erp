...
@php
    ...
    $fields = json_decode('[{"label":"Bank Name","name":"bank_name","placeholder":"HDFC Bank","type":"text"},{"label":"Account Name","name":"account_name","placeholder":"Cholavin Foods","type":"text"},{"label":"Account Number","name":"account_number","placeholder":"Enter account number","type":"text"},{"label":"IFSC Code","name":"ifsc_code","placeholder":"HDFC0001234","type":"text"},{"label":"Branch","name":"branch","placeholder":"Chennai","type":"text"},{"label":"Opening Balance","name":"opening_balance","placeholder":"0.00","type":"number"},{"label":"Status","name":"status","options":["Active","Inactive"],"type":"select"}]', true);
    ...
    <div class="card erp-panel mb-0">
        <div class="card-body d-flex flex-wrap align-items-center justify-content-between gap-3">
            ...
            <div class="d-flex gap-2">
                @can('bank-accounts.export')<button id="download-pdf" class="btn btn-secondary" type="button"><i class="ri-file-pdf-2-line me-1"></i>PDF</button>@endcan
                @can('bank-accounts.create')<a class="btn btn-primary" href="{{ route('admin.bank-accounts.create') }}"><i class="ri-add-line me-1"></i>Add {{ $module['singular'] }}</a>@endcan
            </div>
        </div>
    </div>
...
$(function () {
    const table = initializeDataTable({
        ...
        url: @json(route('admin.bank-accounts.index')),
        filters: () => ({
    ...
    $('#download-pdf').on('click', () => {
        ...
        window.location.href = @json(route('admin.bank-accounts.pdf')) + '?' + params;
    });
});
</script>
@endpush