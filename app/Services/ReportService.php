...
class ReportService
{
    ...
    public function query(string $report, Request $request): Builder
    {
        ...
        $source = ! $shopId && ! $request->user()?->isSuperAdmin()
            ? $this->emptyReport()
            : match ($report) {
    ...
    private function documents(int $shopId, array $types): Builder
    {
        return DB::table('commercial_documents as documents')
            ...
            ->when($shopId, fn (Builder $query) => $query->where('documents.shop_id', $shopId))
            ->when(session('active_financial_year_id'), fn (Builder $query) => $query->where('documents.financial_year_id', session('active_financial_year_id')))
    ...
    private function stockMovements(int $shopId, int $godownId): Builder
    {
        return DB::table('inventory_movements as movements')
            ...
            ->when($shopId, fn (Builder $query) => $query->where('movements.shop_id', $shopId))
    ...
    private function stockBalances(int $shopId, int $godownId, bool $lowStock, bool $expiry): Builder
    {
        return DB::table('inventory_balances as balances')
            ...
            ->when($shopId, fn (Builder $query) => $query->where('balances.shop_id', $shopId))
    ...
    private function partyBalances(int $shopId): Builder
    {
        return DB::table('parties')
            ->when($shopId, fn (Builder $query) => $query->where('parties.shop_id', $shopId))
    ...
    private function partyLedger(int $shopId, ?int $partyId): Builder
    {
        ...
        $payments = DB::table('payments')
            ...
            ->when($shopId, fn (Builder $query) => $query->where('payments.shop_id', $shopId))
            ->when(session('active_financial_year_id'), fn (Builder $query) => $query->where('payments.financial_year_id', session('active_financial_year_id')))
    ...
    private function dayBook(int $shopId): Builder
    {
        return DB::table('vouchers')
            ...
            ->when(session('active_financial_year_id'), fn (Builder $query) => $query->where('vouchers.financial_year_id', session('active_financial_year_id')))
    ...
    private function activity(int $shopId): Builder
    {
        return DB::table('activity_logs')
            ...
            ->when(session('active_financial_year_id'), fn (Builder $query) => $query->where(fn (Builder $query) => $query->whereNull('activity_logs.financial_year_id')->orWhere('activity_logs.financial_year_id', session('active_financial_year_id'))))
    ...
    private function emptyReport(): Builder
    {
        return DB::table('commercial_documents')
            ->whereRaw('1 = 0')
            ->selectRaw("document_date as report_date, number as reference, '' as party, type as description, 0 as debit, 0 as credit, 0 as amount, status as status");
    }
}
